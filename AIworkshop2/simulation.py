...
def run_financial_simulation(
    ...
    current_fhs = float(summary.get('latest_fhs', 50.0)) # Ensure float
    ...
    for d in range(1, days + 1):
        ...
        income = float(forecast_day_data.get('forecast_income', 0.0))
        expense = float(forecast_day_data.get('forecast_expense', 0.0))
        ...
        fhs_impact = (net_flow / 1000.0) * 0.1 
        current_fhs = max(0.0, min(100.0, current_fhs + fhs_impact))
    ...
    if target_amount is not None and target_amount > 0: # Check for None
    ...
    # === CRITICAL: Ensure all values are python float, NOT numpy types ===
    return {
        ...
        "fhs_change": float(round(current_fhs - float(summary.get('latest_fhs', 50.0)), 2)),
        "time_horizon_months": int(months),
        "goal_status": str(goal_status),
        "simulation_goal": str(params.get('simulation_goal', 'General')),
        "daily_balances": [float(round(b, 2)) for b in daily_balances[::30]], 
        "daily_fhs": [float(round(f, 2)) for f in fhs_scores[::30]] 
    }
...
async def generate_detailed_report(
    ...
    data_str = json.dumps(simulation_data, indent=2, cls=SafeEncoder)
    ...
    if twin_scenarios:
        twin_str = json.dumps(twin_scenarios, indent=2, cls=SafeEncoder)
...
async def generate_general_chat_response(user_question: str) -> str:
    ...
    try:
        ...
        return "Service unavailable."# AI workshop 2/simulation.py
import os
import json
import logging
from typing import Dict, Any, List
