...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Image, TextInput, Alert, ActivityIndicator, Animated } from "react-native";
...
export default function ProfileScreen({ navigation }) {
  ...
  const { accounts, transactions, goals, budgets } = useTransactions();
  ...
  const monthlySpending = useMemo(() => {
    ...
    return transactions
      .filter(t => {
        if (!t.date || (t.type !== 'expend' && t.type !== 'Expense')) return false;
  ...
  const activeGoalsCount = useMemo(() => goals.length, [goals]);
  const monthlyBudget = useMemo(() => budgets.reduce((sum, b) => sum + (parseFloat(b.limit_amount) || 0), 0), [budgets]);

  // Calculate Savings Rate = (Income - Expense) / Income (Naive approximation or just dummy if no income)
  const monthlyIncome = useMemo(() => {
    const now = new Date();
    return transactions
      .filter(t => {
        if (!t.date || (t.type !== 'income' && t.type !== 'Income')) return false;
        const d = new Date(t.date);
        return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear();
      })
      .reduce((sum, t) => sum + (parseFloat(t.amount) || 0), 0);
  ...
  const [userData, setUserData] = useState({
    ...
    monthlyIncome: monthlyIncome || 5000, // Use real calculated or fallback
    savingsGoal: 1000,
  ...
  // Sync userData with Auth updates if needed
  useEffect(() => {
    setUserData(prev => ({
      ...prev,
  ...
  const stats = [
    { label: "Total Transactions", value: transactions.length.toString(), icon: "swap-horizontal" },
    { label: "Monthly Budget", value: `RM ${monthlyBudget.toLocaleString()}`, icon: "cash" },
    { label: "Savings Rate", value: `${savingsRate}%`, icon: "trending-up" },
    { label: "Goals Achieved", value: `${completedGoals}/${activeGoalsCount}`, icon: "target" },
  ];
  ...
  return (
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <AnimatedHeader
        ...
        rightComponent={
          <TouchableOpacity
            ...
            style={{ padding: 8, backgroundColor: isEditing ? colors.error : colors.primary, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 4 }}
          >
      ...
      />
      <Animated.ScrollView
        contentContainerStyle={{ paddingBottom: 100, paddingTop: 70 + insets.top }}
      ...
      >
        <Text style={[styles.title, { color: colors.primary, marginBottom: 20, textShadowColor: colors.primary, textShadowRadius: 10 }]}>My Profile</Text>
